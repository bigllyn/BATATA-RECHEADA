...
import { savePixKey, getSettings, saveProfile } from "./actions";
...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    if (activeTab === "payments") {
      ...
      const result = await saveProfile(name, description);
      if (result.success) {
          ...
          {activeTab === "profile" && (
            <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6 animate-in fade-in duration-300">
              <div className="sm:col-span-3">
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 text-gray-900 focus:outline-none focus:ring-[#6a1b9a] focus:border-[#6a1b9a] sm:text-sm" />
              </div>
              <div className="sm:col-span-3">
                <label className="block text-sm font-medium text-gray-700">Link do Cardápio (Slug)</label>
                <div className="mt-1 flex rounded-md shadow-sm">
                  <span className="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-50 text-gray-500 sm:text-sm">seusite.com/</span>
                  <input type="text" value="gourmet-bites" disabled className="flex-1 block w-full rounded-none rounded-r-md border border-gray-300 py-2 px-3 text-gray-500 bg-gray-50 focus:outline-none sm:text-sm" title="O slug não pode ser alterado por aqui" />
                </div>
              </div>
              <div className="sm:col-span-6">
                <label className="block text-sm font-medium text-gray-700">Descrição Curta</label>
                <textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 text-gray-900 focus:outline-none focus:ring-[#6a1b9a] focus:border-[#6a1b9a] sm:text-sm" />
              </div>
            </div>
          )}
